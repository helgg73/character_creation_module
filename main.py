...
def defence(char_name: str, char_class: str) -> str:
    """Вычисляет результаты защиты."""
    deflect: int = 0
    if char_class == 'warrior':
        deflect = 10 + randint(5, 10)
    if char_class == 'mage':
        deflect = 10 + randint(-2, 2)
    if char_class == 'healer':
        deflect = 10 + randint(2, 5)
    if deflect != 0:
        return (f'{char_name} блокировал {deflect} урона')
    return (f'{char_name} не блокировал урон')


def special(char_name: str, char_class: str) -> str:
    """Вычисляет результаты применения способностей."""
    damage: int = 0
    skill: str = 'Неизвестно'
    if char_class == 'warrior':
        skill = 'Выносливость'
        damage = 80 + 25
    if char_class == 'mage':
    ...
    if char_class == 'healer':
        ...
        damage = 10 + 30
    if damage != 0:
        return (f'{char_name} применил специальное умение «{skill} {damage}»')
